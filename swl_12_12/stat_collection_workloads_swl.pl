...
@benchmarks_complete =( Combination(@apps), Combination(@apps2),Combination(@apps4),Product(\@apps, \@apps2),Product(\@apps, \@apps4),Product(\@apps2, \@apps4));
...
$result_file = "all_results_swls.txt";
...
$sing_apps1 = "/sciclone/data20/fluo/GPU_RESEARCH/research_mafia_experiments/swl_" . $swl1 . "/single_results.txt";
$sing_apps2 = "/sciclone/data20/fluo/GPU_RESEARCH/research_mafia_experiments/swl_" . $swl2 . "/single_results.txt";
...
for ($j = 0; $j <= $#file_list1; $j++)
{
	...
	#print "$file_list[$j]";
	my $sing_ipc1 = `grep -w $app1_name $sing_apps1 | awk '\{print \$2\}'`;
	my $sing_ipc2 = `grep -w $app2_name $sing_apps2 | awk '\{print \$2\}'`;
    ...
    my $app = `grep "statistics" $file_list1[$j] | head -n 1 | awk '\{print \$2\}'`;
    $app = trim($app); 
    #chomp($app);
    $app = substr($app,0,4); 
