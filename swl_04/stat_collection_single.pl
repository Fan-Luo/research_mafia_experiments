...
for ($j = 0; $j <= $#file_list; $j++)
{
    ...
    my $stall_warps = `grep "Stall:" $file_list[$j] | tail -n 3 | head -n 1 | awk -F '[: \t]' '\{print \$2\}'`;
	chomp($stall_warps);
	push @globalstallwarps, $stall_warps;	

    my $stall_dramfull = `grep "gpu_stall_dramfull" $file_list[$j] | tail -n 1 | awk '\{print \$3\}'`;
	chomp($stall_dramfull);
	push @globalstalldramfull, $stall_dramfull;	

    my $stall_icnt2sh = `grep "gpu_stall_icnt2sh" $file_list[$j] | tail -n 1 | awk '\{print \$3\}'`;
	chomp($stall_icnt2sh);
	push @globalstallicnt2sh, $stall_icnt2sh;	


	my $insn = `grep "gpu_tot_sim_insn =" $file_list[$j] | tail -n 1 | awk '\{print \$3\}'`;
	chomp($insn);
	push @globalinsn, $insn;	
	...
	my $averagemrqlatency = `grep "averagemrqlatency_1" $file_list[$j] |  tail -n 1 |  awk '\{print \$3\}'`;
	chomp($averagemrqlatency);
	push @globalaveragemrqlatency, $averagemrqlatency;
	...
	my @bw_dist_3 = `grep "bw_dist" $file_list[$j] | tail -n 6 |  awk '\{print \$6\}'`;
	chomp(@bw_dist_3);
	push @globalbw_dist_3, [avg(\@bw_dist_3)];
	
	my @bw_dist_4 = `grep "bw_dist" $file_list[$j] | tail -n 6 |  awk '\{print \$7\}'`;
	chomp(@bw_dist_4);
	...
	my $mpki = `grep  "Avg_MPKI_Stream1" $file_list[$j] | tail -n  1 | awk '\{print \$2\}'`;
	chomp($mpki);
	push @globalmpki, $mpki;
	
	# my $L2missrate = `grep  "L2 Cache Total Miss Rate" $file_list[$j] | tail -n  1 | awk '\{print \$7\}'`;
	# chomp($L2missrate);
	# push @globalL2missrate, $L2missrate;
	
 #    my $L2ac_1 = `grep  "Stream 1: Misses" $file_list[$j] | tail -n  1 | awk '\{print \$5\}'`;
	# chomp($L2ac_1);
	# push @globalL2ac_1, $L2ac_1;
        
 #    my $L2ac_2 = `grep  "Stream 2: Misses" $file_list[$j] | tail -n  1 | awk '\{print \$5\}'`;
	# chomp($L2ac_2);
	# push @globalL2ac_2, $L2ac_2;

	# my $wa_1 = `grep global_avg_wa_app_1 $file_list[$j] | tail -n 1 | awk '\{print \$3\}'`;
	# chomp($wa_1);
	# push @global_wa_1, $wa_1;	
	
	# my $tw_1 = `grep global_avg_tw_app_1 $file_list[$j] | tail -n 1 | awk '\{print \$3\}'`;
	# chomp($tw_1);
	# push @global_tw_1, $tw_1;		
...
for ($k = 0;$k <= $#file_list;$k++)
{
	...
	print OUTPUT "$globalmpki[$k] \t";
	# print OUTPUT "$globalL2missrate[$k] \t";
	
	# print OUTPUT "$global_wa_1[$k] \t";
	# print OUTPUT "$global_tw_1[$k] \t";
